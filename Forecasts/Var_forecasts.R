##This script performs the var forecast###
...
######################################################################################################
### FORECAST: 1-day horizon VaR for 5% and 1% Levels ###
######################################################################################################
...
#aligning by putting 9 NAs in
#VaR_full_0.05 <- c(rep(NA, 9), VaR_full_0.05)
#VaR_full_0.01 <- c(rep(NA, 9), VaR_full_0.01)
...
#head(test_aapl$log_x, - (forecast_horizon - 1))),
#### PLOT the above
...
# Define the standardised skewed student's t-distribution function
standardised_student_t <- function(nu, gamma) {
  student.t(nu = nu, gamma = gamma, mu = 0, sigma = 1)
}

######################################################################################################
### Out of Sample VaR Forecasts (10-day ahead) Function ###
######################################################################################################
har_garch_10day_var_forecast <- function(params, model, forecast_horizon, results, train_data, test_data, alpha_0.05 = 0.05, alpha_0.01 = 0.01) {
  # Extracting fitted values
  z <- results$fitted_models[[model]]$fitted_values$z
  ut <- results$fitted_models[[model]]$fitted_values$ut
  log_sigma <- results$fitted_models[[model]]$fitted_values$log_sigma2
  ...
  rolling_forecasts <- numeric(nrow(test_data) - forecast_horizon + 1)
  rolling_var_10day_0.05 <- numeric(nrow(test_data) - forecast_horizon + 1)
  rolling_var_10day_0.01 <- numeric(nrow(test_data) - forecast_horizon + 1)
  ...
  for (start in 1:(nrow(test_data) - forecast_horizon + 1)) { 
    ...
    # Calculate VaR using standardized skewed student's t-distribution
    sigma_t <- sqrt(exp(log_sigma_t))
    skew <- 0  # skewness parameter
    VaR_dist <- standardised_student_t(df, skew)
    
    rolling_var_10day_0.05[start + forecast_horizon - 1] <- qghyp(1 - alpha_0.05, VaR_dist) * sigma_t * sqrt(10)
    rolling_var_10day_0.01[start + forecast_horizon - 1] <- qghyp(1 - alpha_0.01, VaR_dist) * sigma_t * sqrt(10)
  }
  
  return(list(rolling_forecasts = rolling_forecasts, VaR_10day_0.05 = rolling_var_10day_0.05, VaR_10day_0.01 = rolling_var_10day_0.01))
}

######################################################################################################
### FORECAST: 10-day horizon VaR for 5% and 1% Levels ###
######################################################################################################
params <- all_results$AAPL.csv$fitted_models$realised_har_garch$optimal_params
forecast_horizon <- 10  # 10-day ahead forecast
alpha_0.05 <- 0.05
alpha_0.01 <- 0.01 
var_10day_forecast_results <- har_garch_10day_var_forecast(params, "realised_har_garch", forecast_horizon, all_results$AAPL.csv, train_aapl, test_aapl, alpha_0.05, alpha_0.01)

# Extracting forecasts and VaR
rolling_forecasts <- var_10day_forecast_results$rolling_forecasts

VaR_10day_0.05 <- -var_10day_forecast_results$VaR_10day_0.05
VaR_10day_0.05 <- VaR_10day_0.05[VaR_10day_0.05 != 0]

VaR_10day_0.01 <- -var_10day_forecast_results$VaR_10day_0.01
VaR_10day_0.01 <- VaR_10day_0.01[VaR_10day_0.01 != 0]
...
combined_data_var_10day <- data.frame(
  Day = 1:(train_data_len + test_data_len),
  Log_x = c(exp(train_aapl$log_x), exp(test_aapl$log_x)),
  Type = rep(c("Train", "Test"), times = c(train_data_len, test_data_len))
)
...
# Add 1-day VaR (5% level) for test period
combined_data_var_10day$VaR_10day_0.05 <- NA
combined_data_var_10day$VaR_10day_0.05[(train_data_len + forecast_horizon):(train_data_len + forecast_horizon + length(VaR_10day_0.05) - 1)] <- VaR_10day_0.05

# Add 1-day VaR (1% level) for test period
combined_data_var_10day$VaR_10day_0.01 <- NA
combined_data_var_10day$VaR_10day_0.01[(train_data_len + forecast_horizon):(train_data_len + forecast_horizon + length(VaR_10day_0.01) - 1)] <- VaR_10day_0.01

######################################################################################################
### IN-SAMPLE: 10-day horizon VaR for 5% and 1% Levels ###
######################################################################################################
# Define in-sample VaR calculation function
in_sample_all_models_10day_var <- function(model, params, results, alpha_0.05 = 0.05, alpha_0.01 = 0.01) {
  # Extracting fitted values
  sigma2 <- results$fitted_models[[model]]$fitted_values$sigma2
  z <- results$fitted_models[[model]]$fitted_values$z
  ...
  skew <- fit@gamma[1]  
  # Initialize a list to store in-sample VaR
  in_sample_var_10day_0.05 <- numeric(length(sigma2))
  in_sample_var_10day_0.01 <- numeric(length(sigma2))
  ...
  for (t in 1:length(sigma2)) {
    ...
    in_sample_var_10day_0.05[t] <- qghyp(1 - alpha_0.05, VaR_dist) * sigma_t * sqrt(10)
    in_sample_var_10day_0.01[t] <- qghyp(1 - alpha_0.01, VaR_dist) * sigma_t * sqrt(10)
  }
  
  return(list(VaR_10day_0.05 = in_sample_var_10day_0.05, VaR_10day_0.01 = in_sample_var_10day_0.01))
}


##### Estimate in-sample VaR
in_sample_var_10day_results <- in_sample_all_models_10day_var("realised_har_garch", params, all_results$AAPL.csv, alpha_0.05, alpha_0.01)

#### Combine in-sample and out-of-sample VaR
VaR_10day_full_0.05 <- c(-in_sample_var_10day_results$VaR_10day_0.05, VaR_10day_0.05)
VaR_10day_full_0.01 <- c(-in_sample_var_10day_results$VaR_10day_0.01, VaR_10day_0.01)

#aligning by putting 9 NAs in
VaR_10day_full_0.05 <- c(rep(NA, 9), VaR_10day_full_0.05)
VaR_10day_full_0.01 <- c(rep(NA, 9), VaR_10day_full_0.01)

VaR_10day_full_0.05 <- VaR_10day_full_0.05[VaR_10day_full_0.05 != 0]
VaR_10day_full_0.01 <- VaR_10day_full_0.01[VaR_10day_full_0.01 != 0]
#VaR_10day_full <- VaR_10day_full[VaR_10day_full != 0]

#### PLOT the above
# Combine training data, test data, VaR, and daily returns for plotting
combined_data_var_10day_full <- data.frame(
  Day = 1:(train_data_len + test_data_len),
  Log_x = c(train_aapl$log_x, test_aapl$log_x),
  Returns = c(train_aapl$returns, test_aapl$returns),
  Type = rep(c("Train", "Test"), times = c(train_data_len, test_data_len))
)

# Add VaR for the full period
combined_data_var_10day_full$VaR_10day_0.05 <- VaR_10day_full_0.05
combined_data_var_10day_full$VaR_10day_0.01 <- VaR_10day_full_0.01
#combined_data_var_10day_full$VaR_10day <- VaR_10day_full

# Plot VaR and daily returns
p_var_full <- ggplot(combined_data_var_10day_full, aes(x = Day)) +
  geom_line(aes(y = Returns, color = "Daily Returns"), linetype = "solid") +
  geom_line(aes(y = VaR_10day_0.05, color = "VaR 10-day 5%"), na.rm = TRUE, linetype = "dotted") +
  geom_line(aes(y = VaR_10day_0.01, color = "VaR 10-day 1%"), na.rm = TRUE, linetype = "dotted") +
  geom_vline(xintercept = train_data_len, linetype = "dashed", color = "black") +
  labs(title = "VaR: 10-Day Forecasts at 5% and 1% Level with Daily Returns", x = "Day", y = "Value") +
  scale_color_manual(values = c("Daily Returns" = "black", "VaR 10-day 5%" = "purple", "VaR 10-day 1%" = "orange")) +
  scale_y_continuous(
    limits = c(-1, 1),  # Adjust these limits based on your data range
    #breaks = seq(-2, 2, by = 0.5),  # Set breaks at 0.5 intervals
    #labels = function(x) paste0(x, "%")  # Add percentage sign to labels
...
# Compute metrics
metrics <- compute_var_metrics(returns_full, VaR_full_0.01, alpha_0.01)
...
VaR_test <- VaRTest(0.01, aapl_data$returns, combined_data_var_full$VaR_0.01, 0.95)
